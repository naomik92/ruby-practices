...
class BowlingObjectTest < Minitest::Test
  def test_point1
    ...
    game = Game.new(marks)
    assert_equal 139, game.calculate_point
  end

  def test_point2
    marks = '6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,X,X'
    game = Game.new(marks)
    assert_equal 164, game.calculate_point
  end

  def test_point3
    marks = '0,10,1,5,0,0,0,0,X,X,X,5,1,8,1,0,4'
    game = Game.new(marks)
    assert_equal 107, game.calculate_point
  end

  def test_point4
    marks = '6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,0,0'
    game = Game.new(marks)
    assert_equal 134, game.calculate_point
  end

  def test_point5
    marks = '6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,1,8'
    game = Game.new(marks)
    assert_equal 144, game.calculate_point
  end

  def test_point6
    marks = 'X,X,X,X,X,X,X,X,X,X,X,X'
    game = Game.new(marks)
    assert_equal 300, game.calculate_point
  end

  def test_point7
    marks = 'X,X,X,X,X,X,X,X,X,X,X,2'
    game = Game.new(marks)
    assert_equal 292, game.calculate_point
  end

  def test_point8
    marks = 'X,0,0,X,0,0,X,0,0,X,0,0,X,0,0'
    game = Game.new(marks)
    assert_equal 50, game.calculate_point
  end
end
