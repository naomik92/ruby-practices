...
class LsCommandTest < Minitest::Test
  def test_non_option
    options = {}
    files = Dir.entries('.').sort.reject
    expected_files_array = [['abc.txt', 'ghi.txt', 'ls_command_test.rb'], ['def.txt', 'jk.txt', '']]
    actual_files_array = LsCommand.new(files, options).build_files.map { |row| row.map(&:rstrip) }
  ...
  def test_l_option
    ...
    actual_files_array = LsCommand.new(files, options).build_files.map { |row| row[0, 2] }
    assert_equal expected_files_array, actual_files_array
  end
